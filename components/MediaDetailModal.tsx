...
const MediaDetailModal: React.FC<MediaDetailModalProps> = ({ items, initialIndex, onClose, onUserClick, session, onDataChange }) => {
  ...
  // Bug fix: disable swipe completely when zoomed
  ...
  return (
    ...
    >
        ...
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
            <img 
                src={item.src} 
                className="w-full h-full object-cover scale-110 blur-[120px] opacity-30 brightness-[0.1]" 
                alt="" 
            />
        ...
        <div className="absolute top-0 inset-x-0 z-[1100] flex items-center justify-between p-4 md:p-8 pt-safe-top">
            <button 
                onClick={handleClose} 
                className="p-3 text-white/80 hover:text-white bg-white/5 hover:bg-white/10 backdrop-blur-3xl rounded-2xl transition-all border border-white/10 active:scale-90 shadow-2xl"
            >
            ...
            <div className="flex items-center gap-3">
                 <button 
                    ...
                    className="p-3 text-white/40 hover:text-red-400 bg-white/5 hover:bg-red-500/10 backdrop-blur-3xl rounded-2xl transition-all border border-white/10 active:scale-90"
                 >
        ...
        {/* Main Content Area */}
        <div 
            className="flex-grow w-full h-full relative overflow-hidden z-10 flex items-center justify-center px-4 py-20 md:px-32 md:py-24"
        ...
        <div className="hidden md:block absolute inset-y-0 left-0 z-[1050] w-24">
            <button 
                ...
                disabled={currentIndex === 0 || isZoomed}
        ...
        <div className="hidden md:block absolute inset-y-0 right-0 z-[1050] w-24">
            <button 
                onClick={goToNext}
                disabled={currentIndex === items.length - 1 || isZoomed}
                className="w-full h-full flex items-center justify-center text-white/10 hover:text-pink-500 hover:bg-white/5 transition-all disabled:opacity-0"
            >
        ...
        {/* Interaction HUD - Repositioned for focus */}
        <div className={`absolute right-4 md:right-10 top-1/2 -translate-y-1/2 z-[1100] flex flex-col items-center gap-5 md:gap-7 pointer-events-auto transition-opacity duration-300 ${isZoomed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            <div className="flex flex-col items-center">
                <button 
                    onClick={(e) => { e.stopPropagation(); handleLikeAction(); }}
                    className={`p-4 md:p-5 rounded-[2rem] bg-black/40 backdrop-blur-3xl border transition-all active:scale-50 shadow-2xl ${isLiked ? 'text-pink-500 border-pink-500/50' : 'text-white border-white/10'}`}
                >
                    <HeartIcon filled={isLiked} className="w-7 h-7 md:w-8 md:h-8" />
                </button>
                <span className="text-[10px] font-black text-white/70 mt-2 uppercase tracking-tight">{likeCount > 999 ? (likeCount/1000).toFixed(1) + 'k' : likeCount}</span>
            </div>

            <div className="flex flex-col items-center">
                <button 
                    onClick={(e) => { e.stopPropagation(); setIsDrawerOpen(true); }}
                    className="p-4 md:p-5 rounded-[2rem] bg-black/40 backdrop-blur-3xl border border-white/10 text-white transition-all active:scale-75 shadow-2xl"
                >
                    <ChatIcon className="w-7 h-7 md:w-8 md:h-8" />
                </button>
                <span className="text-[10px] font-black text-white/70 mt-2 uppercase tracking-tight">Info</span>
            </div>

            <div className="flex flex-col items-center">
                <button 
                    onClick={(e) => { e.stopPropagation(); setIsTipModalOpen(true); }}
                    className="p-4 md:p-5 rounded-[2rem] bg-black/40 backdrop-blur-3xl border border-white/10 text-yellow-500 transition-all active:scale-75 shadow-2xl"
                >
                    <GiftIcon className="w-7 h-7 md:w-8 md:h-8" />
                </button>
                <span className="text-[10px] font-black text-white/70 mt-2 uppercase tracking-tight">Gift</span>
            </div>
            ...
            <div className="flex flex-col items-center">
                ...
                <span className="text-[10px] font-black text-white/70 mt-2 uppercase tracking-tight">Link</span>
            </div>
        </div>

        {/* Bottom Info Bar */}
        <div className={`absolute bottom-0 inset-x-0 z-[1050] pointer-events-none transition-opacity duration-300 ${isZoomed ? 'opacity-0' : 'opacity-100'}`}>
            <div className="max-w-7xl mx-auto px-6 md:px-12 pb-8 md:pb-12 flex items-end">
                <div className="flex-grow max-w-[80%] md:max-w-[60%] pointer-events-auto flex flex-col gap-4 animate-slide-up">
                    <div 
                    ...
                    >
                        <Avatar src={item.author_avatar} alt={item.author} size="lg" className="ring-2 ring-pink-500/50 group-hover:scale-105 transition-transform" />
                        <div>
                            <h3 className="text-white font-black text-xl font-orbitron tracking-tight">@{item.author}</h3>
                    ...
                    <p className="text-white/90 text-sm md:text-base line-clamp-2 leading-relaxed font-light drop-shadow-lg">
                        {item.description}
                    </p>
                    
                    <div className="flex flex-wrap gap-2">
                        {item.tags?.slice(0, 6).map(tag => (
                            <span key={tag} className="text-pink-400 font-black text-[9px] uppercase tracking-widest bg-pink-950/20 px-3 py-1 rounded-full border border-pink-500/10">#{tag}</span>
                        ))}
                    </div>
                </div>
            </div>
        ...
        {isDrawerOpen && (
            <div className="fixed inset-0 z-[1200] flex animate-fade-in">
                ...
                <div className="w-full md:w-[460px] bg-[#050505] h-full shadow-[-20px_0_60px_rgba(0,0,0,0.8)] border-l border-white/5 overflow-y-auto custom-scrollbar animate-slide-left">
                    <div className="sticky top-0 z-50 p-6 md:p-8 bg-[#050505]/95 backdrop-blur-xl flex items-center justify-between border-b border-white/5 pt-safe-top">
                        <h3 className="text-white font-black uppercase tracking-[0.3em] text-[10px]">Neural Transmission</h3>
                        <button onClick={() => setIsDrawerOpen(false)} className="p-2 text-gray-500 hover:text-white bg-white/5 rounded-xl transition-colors"><CloseIcon className="w-5 h-5" /></button>
                    </div>
                    <MediaSidebar 
                        item={item} 
                        session={session} 
