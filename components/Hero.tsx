  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-[#050505] z-0 pointer-events-none">
          <div className="absolute inset-0 opacity-30 bg-gradient-to-tr from-pink-900 via-purple-900 to-cyan-900 animate-gradient-xy"></div>
      ...
      <div className="absolute top-1/4 left-1/4 w-2 h-2 bg-pink-500 rounded-full animate-float opacity-50"></div>
      <div className="absolute top-3/4 right-1/4 w-3 h-3 bg-cyan-500 rounded-full animate-float-delayed opacity-50"></div>
      ...
      <div className="relative z-10 container mx-auto px-4 animate-fade-in">
        ...
        >
          {APP_CONFIG.name}<span className="text-cyan-500 mx-1 animate-pulse">{APP_CONFIG.nameSuffix}</span>
        </h1>
        ...
        <div className="flex flex-wrap justify-center gap-6 text-xs font-semibold tracking-widest text-gray-500 uppercase">
            {APP_CONFIG.hero.tags.map((tag, i) => (
                <React.Fragment key={tag}>
                    <span className="hover:text-pink-400 transition-colors cursor-default border border-gray-800 bg-gray-900/50 px-3 py-1 rounded-md hover:border-pink-500/50 hover:shadow-[0_0_15px_rgba(236,72,153,0.2)]">{tag}</span>
                </React.Fragment>
            ))}
        </div>
      </div>
